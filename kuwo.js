...
if (url.indexOf('/mobi.s') !== -1) {
    ...
    let decryptedString;
    try {
        const decryptedData = this.aFunc5(encryptedData, key);
    ...
    q = Buffer.from(this.bFunc2(Buffer.from(params.toString()), key)).toString('base64');
    url = request.searchParams.set('q').toString();
    $.done({url: url});
}
